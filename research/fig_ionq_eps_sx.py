...
fig_ionq_eps_sx.py
------------------
Generates a publication-quality IEEE TQE single-column figure showing
Canary-recovered single-qubit gate error ε_sx across 20 qubits on
IonQ Forte-1 (qpu.forte-1), with the IonQ published device specification
overlaid.

Usage
-----
    python fig_ionq_eps_sx.py

Reads  : research/data/hardware/ionq_canary_allq_0-19_20260810T213159.json
Writes : research/figures/fig_ionq_eps_sx.pdf
         research/figures/fig_ionq_eps_sx.png
"""
...
N_QUBITS  = len(d['qubits'])                        # 20
published = d['published_device_eps_1q']             # 2.0e-4
...
ratio = eps / published
outlier_mask = ratio > 2.0          # only Q3 in this dataset

# ── colour palette (dull blue/grey, IEEE-friendly) ────────────────────────
C_NORMAL  = '#3A6B9E'    # dull slate blue  — qubits within 2× spec
C_OUTLIER = '#9E3A3A'    # dull brick red   — Q3
C_SPEC    = '#1A3050'    # dark navy        — published spec line
C_BAND    = '#C8D8E8'    # very light blue  — ±2× band fill
C_AXIS    = '#2A2A2A'

# ── figure layout ─────────────────────────────────────────────────────────
# IEEE TQE single column: 3.5" wide; 4.2" tall gives comfortable aspect ratio
FW, FH   = 7.16, 3.60
...
# ── qubit-index labels for elevated qubits ────────────────────────────────
for q in qubits:
...
# ══════════════════════════════════════════════════════════════════════════
# RIGHT PANEL — histogram of ε_sx / ε_published
# ══════════════════════════════════════════════════════════════════════════
ax2 = axes[1]
...
# stack individual points into horizontal histogram manually
# (so we can colour individual qubits)
